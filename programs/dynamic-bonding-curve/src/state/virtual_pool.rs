...
impl VirtualPool {
    ...
    pub fn apply_swap_result(
        ...
        if fee_mode.fees_on_base_token {
            self.trading_base_fee = self.trading_base_fee.safe_add(partner_fee)?;
            ...
            self.trading_quote_fee = self.trading_quote_fee.safe_add(partner_fee)?;
